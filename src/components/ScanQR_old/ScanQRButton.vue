...
const connect = async () => {
  try {
    emit('connect');
    const response = await ScanAPIs.connect();
    if (response.data) {
...
const scan = async () => {
  try {
    ...
    const { data } = await ScanAPIs.scanQr(controller.value.signal);
    if (data) {
...
<template>
  ...
  <ErrorDialog
    ...
    title="QR scanner not detected"
    description="The QR scanner is not detected or not identified in our system."
  />
    ...
    <template #message>
      <div class="d-flex align-items-center justify-content-between col-12">
        <div class="d-flex gap-2 align-items-center">
          ...
          <div class=""><i class="ri-rfid-line"></i>QR Scanning...</div>
        </div>
        <div class="d-flex gap-2 align-items-center">
