...
import ScanAPIs from '@/services/scanner_old.service';
...
const connect = async () => {
  try {
    ...
    //   life: 3000,
    // });
    // const response = props.bulk
    //   ? await ScanAPIs.connectHandheld()
    //   : await ScanAPIs.connect();
    const response =
      props.bulk && !props.powerbank // temporary
        ? await ScanAPIs.connectHandheld()
        : await ScanAPIs.connect();
    if (response.data) {
...
const scan = async () => {
  try {
    ...
    const { data } = await ScanAPIs.scan(controller.value.signal);
    if (data) {
...
const bulkScan = () => {
  ...
  //   socket.value?.send('v2/bulkscan');
  // };
  socket.value.onopen = (event) => {
    console.log('open', event);
    socket.value?.send(!props.powerbank ? 'v2/bulkscan' : 'v1/bulkscan');
    // socket.value?.send('v2/bulkscan');
    // socket.value?.send('v1/bulkscan');
  };
...
<template>
  ...
  <ErrorDialog
    ...
    description="The scanner driver is not installed or not running properly in your device. Please contact your administrator"
  />

  <ErrorDialog
    v-model:visible="dialog.scannerNotDetected"
    title="RFID scanner not detected"
    description="The RFID scanner is not detected or not identified in our system."
  />
