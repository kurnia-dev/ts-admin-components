...
export default {
  name: 'OverlayPanel',
  inheritAttrs: false,
  emits: ['show', 'hide', 'update:selectedItems'],
  props: {
    dataKey: {
      type: String,
      required: false, // true,
  ...
  watch: {
    ...
    selectedItems: {
      immediate: true,
      handler(newVal) {
        this.optionsModel = this.setSelectedOptions(newVal);
      },
    },
  },
  selfClick: false,
  ...
  mounted() {
    ...
    // this.optionsModel = this.options.map((option) => {
    //   return {
    //     ...option,
    //     selected: false,
    //   };
  ...
  methods: {
    /**
     * Sync selected options with selected items
     *
     * @param {*} selectedItems
     */
    setSelectedOptions(selectedItems) {
      const selectedOptions = this.options.map((option) => {
        if (this.optionValue) {
          if (selectedItems.includes(option[this.optionValue])) {
            return {
              ...option,
              selected: true,
            };
          ...
          const isSelected = selectedItems.some(
            (item) => item[this.dataKey] === option[this.dataKey]
          );
          if (isSelected) {
            return {
              ...option,
    ...
    isTargetClicked(event) {
      return (
        ...
        (this.eventTarget === event.target ||
          this.eventTarget.contains(event.target))
      );
    },
    containerRef(el) {
      this.container = el;
          ...
          >
            <template #option="slotProps">
              <div class="d-flex align-items-center">
                <Checkbox
                  v-model="slotProps.option.selected"
                  :binary="true"
                  class="me-2"
                  disabled
                />
