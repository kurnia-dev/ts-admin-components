...
import NameTag from '@/components/NameContainer.vue';
import { TreeNode } from 'primevue/tree';
import { SelectGroupProps, SelectGroupEmits } from '@/components/ts-components/selectgroup/SelectGroup.vue.d';
...
watch(() => props.showDialog, (state: boolean | undefined) => {
  if (state !== undefined) showSelectGroupDialog.value = state;
});

watch(showSelectGroupDialog, (state: boolean) => {
  /**
   * Set the previous selected groups
   */
  if (state) {
    singleSelectedGroupTemp.value = props.singleValue;
  }

  if (props.showDialog !== undefined) {
    emit('update:showDialog', state);
...
<template>
  ...
  <SelectGroupDialog
    ...
    :selected-groups="
      props.selectionMode === 'checkbox' ? groupKeys : singleValue
    "
